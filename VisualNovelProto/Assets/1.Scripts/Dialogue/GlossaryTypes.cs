...
{
    public int id;          // 0..MaxGlossary-1
    public string label;    // 화면에 표시될 텍스트
    public string desc;     // 설명
    public string thumb;    // 스프라이트 키
    public string category; // 분류
    public string colorHex; // CSV에서 읽어온 색 (#RRGGBB)
}

// 글로서리 수집 상태(비트셋, 재귀/동적할당 없음)
[Serializable]
public struct GlossarySet
{
    const int WordBits = 64;
    const int WordCount = 64; // 64*64=4096개 용어까지
    public ulong w0, w1, w2, w3, w4, w5, w6, w7, w8, w9, w10, w11, w12, w13, w14, w15,
    ...
    static ref ulong Pick(ref GlossarySet s, int idx)
    {
        switch (idx)
        {
